/**
 * @reference https://github.com/ElectronicCats/CayenneLPP/blob/master/decoders/decoder.js
...
function decodeUplink(input) {
    ...
    Object.keys(field_aliases).forEach(function(alias) {
        if (response.hasOwnProperty(alias)) {
            response[field_aliases[alias]] = response[alias];
            delete response[alias];
        }
    });

    // Fix up GPS
    if (response.hasOwnProperty('gps_0')) {
        response['latitude'] = response['gps_0'].latitude;
        response['longitude'] = response['gps_0'].longitude;
        response['altitude'] = response['gps_0'].altitude;
